                ...
                {isEdit ? (
                  <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                    <div style={{ flex:1 }}>
                      ...
                      <input value={editVals[i] || n.value} onChange={e => { const v=e.target.value; setEditVals(p=>({...p,[i]:v})); }}
          ...
          <div>
              ...
              {activePass==="Pass 1" && (
                <div style={{ overflowX:"auto" }}>
                  <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12, minWidth:600 }}>
                    <thead><tr style={{ background:"#f8fafc" }}>{["Pass","Field","Extracted Value","Confidence","Flag"].map(h => <th key={h} style={{ padding:"8px 12px", textAlign:"left", fontWeight:600, color:"#6b7280", borderBottom:"1px solid #f3f4f6" }}>{h}</th>)}</tr></thead>
                    <tbody>
                      {EXTRACTED_FIELDS.map((f, i) => {
                        ...
                        return (
                          <tr key={i} style={{ borderBottom:"1px solid #f9fafb", background:f.flag?"#fffbeb":i%2===0?"white":"#f8fafc" }}>
                            ...
                            <td style={{ padding:"8px 12px" }}><span style={{ display:"flex", alignItems:"center", gap:6 }}><span style={{ width:8, height:8, borderRadius:"50%", background:cm.dot, display:"inline-block" }} /><span style={{ color:cm.color }}>{cm.label}</span></span></td>
              ...
              {activePass==="Pass 2" && (
                <div style={{ overflowX:"auto" }}>
                  <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12, minWidth:700 }}>
                    <thead><tr style={{ background:"#f8fafc" }}>{["Tenant","SF","Rent/SF","Start","Expiry","Use","NAICS","Conf.","Flag"].map(h => <th key={h} style={{ padding:"8px 12px", textAlign:"left", fontWeight:600, color:"#6b7280", borderBottom:"1px solid #f3f4f6", whiteSpace:"nowrap" }}>{h}</th>)}</tr></thead>
                    <tbody>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
              ...
              <div style={{ background:"white", borderRadius:10, border:"1px solid #e5e7eb", padding:16 }}>
                ...
                {[{check:"NOI matches asking price x cap rate",pass:true},{check:"Rent roll SF totals match stated GLA",pass:true},{check:"All expiration dates are future-dated",pass:true},{check:"Amscot rent figure consistent across doc",pass:false}].map((c, i) => (
...
// ── OM MAP — cleaned up, compact layout ────────────────────────────────────
...
function OmMap() {
  ...
  const handleAskAI = () => {
    ...
    setTimeout(() => { setAiLoading(false); setAiInsight(`${sel.label} shows strong fundamentals for a ${sel.type} asset. Cap rate 6.4% — in line with Kendall median. Occupancy 96% above comparable average. 2 inline leases expiring within 9 months. Recommend pursuing — underwrite at $9.2M.`); }, 1200);
  };

  const handleNlSearch = () => {
    if (!nlQuery.trim()) return;
    setNlLoading(true);
    setTimeout(() => { setNlLoading(false); setNlResult("Found 4 matches: Kendall Corners ($9.7M, 6.4% cap, 96% occ), Centre at Cutler Bay ($15.4M, 6.0% cap), Naranja Plaza ($8.9M, 6.8% cap), Meadows Square ($13.8M, 6.1% cap). Kendall Corners is strongest."); }, 1100);
  };

  return (
    <div style={{ display:"flex", height:"100%" }}>
      {/* Compact sidebar */}
      <div style={{ width:240, flexShrink:0, background:"white", borderRight:"1px solid #e5e7eb", display:"flex", flexDirection:"column" }}>
        <div style={{ padding:"10px 12px", borderBottom:"1px solid #f3f4f6" }}>
          <input value={search} onChange={e=>setSearch(e.target.value)}
            style={{ width:"100%", border:"1px solid #e5e7eb", borderRadius:6, padding:"7px 10px", fontSize:12, outline:"none", boxSizing:"border-box" }} placeholder="Search properties..." />
        </div>

        {/* NL query */}
        <div style={{ padding:"10px 12px", borderBottom:"1px solid #f3f4f6" }}>
          <p style={{ fontSize:11, fontWeight:600, color:"#6b7280", margin:"0 0 5px" }}>AI Search</p>
          <div style={{ display:"flex", gap:5 }}>
            <input value={nlQuery} onChange={e=>setNlQuery(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handleNlSearch()}
              style={{ flex:1, border:"1px solid #e5e7eb", borderRadius:6, padding:"6px 8px", fontSize:11, outline:"none" }} placeholder="e.g. grocery-anchored under $10M" />
            <button onClick={handleNlSearch} style={{ fontSize:11, background:"#2E75B6", color:"white", padding:"6px 10px", borderRadius:6, border:"none", cursor:"pointer", fontWeight:600 }}>{nlLoading?"...":"Ask"}</button>
          </div>
          {nlResult && <div style={{ marginTop:6, background:"#eff6ff", border:"1px solid #bfdbfe", borderRadius:5, padding:"6px 8px" }}><p style={{ fontSize:11, color:"#1e3a8a", margin:0, lineHeight:1.4 }}>{nlResult}</p></div>}
        </div>

        {/* Filters */}
        <div style={{ padding:"10px 12px", borderBottom:"1px solid #f3f4f6" }}>
          <p style={{ fontSize:11, fontWeight:600, color:"#6b7280", margin:"0 0 6px" }}>Type</p>
          <div style={{ display:"flex", flexWrap:"wrap", gap:4 }}>
            {["retail","office","medical","mixed"].map(k => (
              <button key={k} onClick={() => setFilters(f=>({...f,[k]:!f[k]}))}
                style={{ fontSize:11, padding:"3px 8px", borderRadius:10, border:"1px solid", borderColor:filters[k]?DOT_COLOR[k]:"#e5e7eb", background:filters[k]?DOT_COLOR[k]+"22":"#f9fafb", color:filters[k]?DOT_COLOR[k]:"#9ca3af", fontWeight:500, cursor:"pointer", textTransform:"capitalize" }}>
                {k}
              </button>
            ))}
          </div>
        ...
        <div style={{ padding:"6px 12px", borderBottom:"1px solid #f3f4f6" }}>
          <p style={{ fontSize:11, color:"#6b7280", margin:0 }}><strong style={{ color:"#374151" }}>{filteredList.length}</strong> of <strong style={{ color:"#374151" }}>127</strong> properties</p>
        </div>
          ...
          {filteredList.map(d => {
            ...
            return (
              <button key={idx} onClick={() => { setSelected(idx===selected?null:idx); setAiInsight(""); }}
                style={{ width:"100%", display:"flex", alignItems:"center", justifyContent:"space-between", padding:"9px 12px", borderBottom:"1px solid #f9fafb", textAlign:"left", background:selected===idx?"#eff6ff":"white", border:"none", cursor:"pointer" }}>
                <div>
                  <p style={{ fontSize:12, fontWeight:500, color:"#111827", margin:0 }}>{d.label}</p>
                  <span style={{ fontSize:10, fontWeight:500, color:DOT_COLOR[d.type], textTransform:"capitalize" }}>{d.type}</span>
                </div>
      ...
      <div style={{ flex:1, position:"relative", overflow:"hidden" }}>
        <svg style={{ position:"absolute", inset:0, width:"100%", height:"100%" }} viewBox="0 0 800 600" preserveAspectRatio="xMidYMid slice">
          ...
          <line x1="120" y1="30" x2="120" y2="580" stroke="#d1c9b8" strokeWidth="2" opacity="0.5" />
          <line x1="280" y1="30" x2="280" y2="590" stroke="#d1c9b8" strokeWidth="2" opacity="0.5" />
          <line x1="60" y1="160" x2="650" y2="160" stroke="#d1c9b8" strokeWidth="1.5" opacity="0.4" />
          <line x1="60" y1="280" x2="650" y2="280" stroke="#d1c9b8" strokeWidth="1.5" opacity="0.4" />
          <line x1="60" y1="400" x2="580" y2="400" stroke="#d1c9b8" strokeWidth="1.5" opacity="0.4" />
          <text x="340" y="260" fontSize="13" fill="#374151" fontWeight="700" textAnchor="middle">Miami</text>
          ...
          <text x="150" y="140" fontSize="10" fill="#9ca3af" textAnchor="middle">Plantation</text>
          <text x="350" y="530" fontSize="10" fill="#9ca3af" textAnchor="middle">Homestead</text>
          <text x="730" y="300" fontSize="11" fill="#4a90b8" textAnchor="middle" transform="rotate(-90,730,300)">Atlantic Ocean</text>
        </svg>
        ...
        {MAP_DOTS.map((d, i) => {
          ...
          return (
            <button key={i} onClick={() => { setSelected(i===selected?null:i); setAiInsight(""); }}
              ...
              <div style={{ width:14, height:14, borderRadius:"50%", border:"2px solid white", background:DOT_COLOR[d.type], transform:selected===i?"scale(1.6)":"scale(1)", boxShadow:"0 1px 4px rgba(0,0,0,0.3)", transition:"transform 0.15s" }} />
            </button>
          );
        })}

        {/* Property popup — compact */}
        {sel && (
          <div style={{ position:"absolute", left:sel.x>60?"auto":`calc(${sel.x}% + 14px)`, right:sel.x>60?`calc(${100-sel.x}% + 14px)`:"auto", top:12, width:220, zIndex:30, background:"white", borderRadius:10, border:"1px solid #e5e7eb", boxShadow:"0 6px 20px rgba(0,0,0,0.12)", padding:14 }}>
            <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:6, marginBottom:8 }}>
              <p style={{ fontSize:13, fontWeight:700, color:"#111827", margin:0, lineHeight:1.3 }}>{sel.label}</p>
              <div style={{ display:"flex", alignItems:"center", gap:4, flexShrink:0 }}>
                <Badge label={sel.type} color={DOT_COLOR[sel.type]} bg={DOT_COLOR[sel.type]+"22"} />
                <button onClick={() => { setSelected(null); setAiInsight(""); }} style={{ width:18, height:18, borderRadius:"50%", background:"#f3f4f6", border:"none", cursor:"pointer", fontSize:11, color:"#6b7280", display:"flex", alignItems:"center", justifyContent:"center" }}>×</button>
              </div>
            </div>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:6, marginBottom:10 }}>
              {[["GLA","34,306 SF"],["Price","$9.7M"],["Cap","6.4%"],["Occ","96%"]].map(([k,v]) => (
                <div key={k} style={{ background:"#f8fafc", borderRadius:5, padding:"6px 8px" }}>
                  <p style={{ fontSize:10, color:"#9ca3af", margin:0 }}>{k}</p>
                  <p style={{ fontSize:12, fontWeight:600, color:"#111827", margin:0 }}>{v}</p>
                </div>
              ))}
            </div>
            {aiLoading && <p style={{ fontSize:11, color:"#2E75B6", textAlign:"center", margin:"0 0 8px" }}>Analyzing...</p>}
            {aiInsight && <div style={{ background:"#eff6ff", border:"1px solid #bfdbfe", borderRadius:7, padding:8, marginBottom:8 }}><p style={{ fontSize:11, color:"#1e3a8a", margin:0, lineHeight:1.4 }}>{aiInsight}</p></div>}
            <div style={{ display:"flex", gap:6 }}>
              <button onClick={handleAskAI} style={{ flex:1, fontSize:11, padding:"6px 0", borderRadius:5, border:"1px solid #bfdbfe", background:"#eff6ff", color:"#2E75B6", cursor:"pointer", fontWeight:500 }}>AI Analyze</button>
              <button style={{ flex:1, fontSize:11, padding:"6px 0", borderRadius:5, border:"none", background:"#2E75B6", color:"white", cursor:"pointer", fontWeight:500 }}>View Record</button>
            </div>
          </div>
        )}

        {/* Legend — compact */}
        <div style={{ position:"absolute", bottom:12, left:12, zIndex:20, background:"white", borderRadius:8, border:"1px solid #e5e7eb", padding:"8px 12px" }}>
          <p style={{ fontSize:10, fontWeight:600, color:"#6b7280", margin:"0 0 6px", textTransform:"uppercase", letterSpacing:1 }}>Type</p>
          ...
          <div style={{ borderTop:"1px solid #f3f4f6", marginTop:6, paddingTop:6 }}>
            <button onClick={() => setHeatmap(!heatmap)} style={{ width:"100%", fontSize:10, padding:"4px 0", borderRadius:4, border:"1px solid", borderColor:heatmap?"#fed7aa":"#e5e7eb", background:heatmap?"#fff7ed":"#f9fafb", color:heatmap?"#ea580c":"#6b7280", cursor:"pointer", fontWeight:600 }}>
        ...
        {heatmap && (
          <svg style={{ position:"absolute", inset:0, width:"100%", height:"100%", pointerEvents:"none", zIndex:3 }}>
            <defs><radialGradient id="hm1" cx="62%" cy="54%" r="18%"><stop offset="0%" stopColor="#ef4444" stopOpacity="0.35" /><stop offset="100%" stopColor="#ef4444" stopOpacity="0" /></radialGradient></defs>
...
function OmLibrary() {
  ...
  const handleAiSearch = () => {
    ...
    setTimeout(() => { setAiLoading(false); setAiResult(`Found 3 properties matching "${aiQuery}": Kendall Corners (96% occ, $9.7M, Active Pursuit), Meadows Square (97% occ, $13.8M, Passed), Centre at Cutler Bay (95% occ, $15.4M, New). Kendall Corners is the strongest active opportunity.`); }, 1300);
  };

  return (
    <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden", background:"#f8fafc" }}>
      <div style={{ padding:"14px 20px", background:"white", borderBottom:"1px solid #e5e7eb" }}>
        <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:8, flexWrap:"wrap" }}>
          <div style={{ flex:1, display:"flex", alignItems:"center", gap:8, border:"1px solid #e5e7eb", borderRadius:8, padding:"7px 12px", background:"white", maxWidth:320 }}>
            <svg width="14" height="14" fill="none" stroke="#9ca3af" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
          ...
          <button onClick={() => setAiSearch(!aiSearch)} style={{ fontSize:12, padding:"7px 12px", borderRadius:8, border:"1px solid", borderColor:aiSearch?"#bfdbfe":"#e5e7eb", background:aiSearch?"#eff6ff":"#f9fafb", color:aiSearch?"#2E75B6":"#6b7280", fontWeight:600, cursor:"pointer", whiteSpace:"nowrap" }}>AI Smart Search</button>
          {statuses.map(s => (
            <button key={s} onClick={() => setSelectedStatus(s)} style={{ fontSize:12, border:"1px solid", borderRadius:20, padding:"4px 12px", fontWeight:500, cursor:"pointer", whiteSpace:"nowrap", borderColor:selectedStatus===s?"#1B2A4A":"#e5e7eb", background:selectedStatus===s?"#1B2A4A":"#f9fafb", color:selectedStatus===s?"white":"#6b7280" }}>{s}</button>
          ))}
        </div>
        {aiSearch && (
          <div style={{ background:"#eff6ff", border:"1px solid #bfdbfe", borderRadius:8, padding:10 }}>
            <div style={{ display:"flex", gap:8 }}>
              <input value={aiQuery} onChange={e=>setAiQuery(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handleAiSearch()} style={{ flex:1, border:"1px solid #bfdbfe", borderRadius:6, padding:"7px 12px", fontSize:13, outline:"none", background:"white" }} placeholder="e.g. grocery-anchored retail over 90% occupancy" />
              <button onClick={handleAiSearch} style={{ background:"#2E75B6", color:"white", fontSize:12, padding:"7px 14px", borderRadius:6, border:"none", cursor:"pointer", fontWeight:600 }}>{aiLoading?"Searching...":"Search"}</button>
            </div>
            {aiResult && <div style={{ marginTop:8, background:"white", border:"1px solid #bfdbfe", borderRadius:6, padding:8 }}><p style={{ fontSize:12, color:"#1e3a8a", margin:0, lineHeight:1.5 }}>{aiResult}</p></div>}
          </div>
        )}
      </div>
      <div style={{ padding:"5px 20px", background:"white", borderBottom:"1px solid #f3f4f6" }}>
          ...
          <tbody>
            {filtered.map((r, i) => (
              <tr key={i} style={{ borderBottom:"1px solid #f3f4f6", cursor:"pointer" }}
                ...
                <td style={{ padding:"11px 16px", fontWeight:500, color:"#111827" }}>{r.name}</td>
                <td style={{ padding:"11px 16px" }}><Badge label={r.type} color={r.typeColor} bg={r.typeColor+"18"} /></td>
                <td style={{ padding:"11px 16px", color:"#6b7280" }}>{r.city}</td>
                <td style={{ padding:"11px 16px", color:"#374151" }}>{r.gla}</td>
                <td style={{ padding:"11px 16px", fontWeight:500, color:"#111827" }}>{r.price}</td>
                <td style={{ padding:"11px 16px", color:"#374151" }}>{r.cap}</td>
                <td style={{ padding:"11px 16px", color:"#374151" }}>{r.occ}</td>
                <td style={{ padding:"11px 16px", color:"#6b7280", fontSize:12 }}>{r.anchor}</td>
                <td style={{ padding:"11px 16px" }}><Badge label={r.status} color={r.statusColor} bg={r.statusBg} /></td>
                <td style={{ padding:"11px 16px", fontSize:11, color:"#9ca3af" }}>{r.date}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
function TenantExpirationTracker() {
  ...
  const handleAI = () => {
    setAiLoading(true);
    setTimeout(() => { setAiLoading(false); setAiInsight("Top prospect: Coastal Coffee Co. — lease expires in 70 days at Kendall Corners, 1,800 SF at $34/SF. Strong growth (6 locations). BurgerFi is second-best — 8 FL locations. Recommend outreach to both within 2 weeks."); }, 1300);
  };
        ...
        <div>
          <div style={{ background:"white", borderRadius:12, border:"1px solid #e5e7eb", marginBottom:20 }}>
            ...
            <div style={{ padding:16 }}>
              {!aiInsight && !aiLoading && <p style={{ fontSize:12, color:"#9ca3af", textAlign:"center", margin:0 }}>Click Run AI Match to get a ranked recommendation.</p>}
